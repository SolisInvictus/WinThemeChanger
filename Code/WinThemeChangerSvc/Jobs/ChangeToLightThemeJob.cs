...
{
    class ChangeToLightThemeJob : IJob
    {
        ...
        private void Change()
        {
            try
            {
                ...
                if (Settings.GetInstance().LightChangeWindowMode)
                    registryKey.SetValue(Consts.SYSTEM_SIDE_LIGHT_THEME_KEY, 1);
                if (Settings.GetInstance().LightChangeApplicationMode)
                    registryKey.SetValue(Consts.APPLICATION_SIZE_LIGHT_THEME_KEY, 1);
            }
            catch (Exception e)
            {
#if DEBUG
